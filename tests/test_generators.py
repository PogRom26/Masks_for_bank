...
def test_filter_by_currency(data_for_tests):
    ...
    assert list(filter_by_currency(data_for_tests, code="USD")) == [
        {
            "id": 939719570,
            "state": "EXECUTED",
            "date": "2018-06-30T02:08:58.425572",
    ...
    assert list(filter_by_currency(data_for_tests, code="RUB")) == [
        {
            "id": 873106923,
            "state": "EXECUTED",
            "date": "2019-03-23T01:09:46.296404",
...
def test_filter_by_currency_without_code(data_for_tests, empty_data):
    """Проверка, что функция правильно обрабатывает случаи,
    когда транзакции в заданной валюте отсутствуют или список пуст."""

    assert (
        next(filter_by_currency(data_for_tests, code="ass"))
        == "В списке операций отсутствует запрашиваемая валюта или список операций"
    )
    assert (
        next(filter_by_currency(empty_data, ""))
        == "В списке операций отсутствует запрашиваемая валюта или список операций"
    )
...
def test_card_number_generator_correct_format():
    ...
    Проверка на наличие трех пробелов в номере карты и их расположение"""
...
def test_card_number_generator_():
    ...
    assert (
        next(card_number_generator(1, 999999999999999999))
        == "Выбранное число не входит в диапазон от 1 до 9999.9999.9999.9999"
    )
